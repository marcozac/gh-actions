...
try {
    genMatrix(JSON.parse(coreExports.getInput('object'))).forEach(([key, matrix]) => {
        coreExports.setOutput(key, matrix);
    });
}
catch (error) {
    coreExports.setFailed(error.message);
}
function genMatrix(input) {
    return Object.entries(input).map(([key, sources]) => [
        key,
        sources.flatMap((entry) => {
            const { name } = entry, obj = __rest(entry, ["name"]);
            return objectToMatrix(obj).map((item) => (Object.assign({ name }, item)));
        }),
    ]);
}
